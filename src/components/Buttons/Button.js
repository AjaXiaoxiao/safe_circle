...
const Button = ({title, color, icon, width}) => {
  return <StyledButton type="button" color={color} width={width} iconOnly={!!icon}> {icon ? <img src={getIcon(icon)} alt={`${icon} icon`} /> : title}</StyledButton>;
};
...
const StyledButton = styled.button`
  ...
  border: none;
  background-color: ${({ color }) => getColor(color).base};
